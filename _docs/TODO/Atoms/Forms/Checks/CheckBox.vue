<template>
    <div>
        <y-modal-info :id="'modal-' + fieldId" :size="modalSizing" :styling="modalStyling" :title="modalTitle">
            {{ modalDescription }}
        </y-modal-info>

        <ValidationProvider v-slot="{ dirty, valid, invalid, errors }" :name="label" :rules="rules">
            <label :class="styleClass(dirty, invalid)" class="y-checkbox--normal">
                <div class="checkbox">
                    <input :id="fieldId" v-model="state.value" class="checkbox-input" name="dasffd" type="checkbox" @change="onChange($event)" />
                ...
                <div class="select-none">
                    <label :for="fieldId" class="flex checkbox--label">
                        ...
                        <label v-if="modalDescription" :for="'modal-' + fieldId" class="icon icon-help icon-small ml-1"></label>
                    </label>
